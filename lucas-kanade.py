...
if __name__ == '__main__':
	...
	# prevgray = cv.cvtColor(prev, cv.COLOR_BGR2GRAY) # needed for Farneback

	show_hsv = True # Draw colors on second window
	while True:
		ret, img = cam.read()

		# gray = cv.cvtColor(img, cv.COLOR_BGR2GRAY) # needed for Farneback

		# flow = cv.calcOpticalFlowFarneback(prevgray, gray, None, 0.6, 3, 15, 3, 5, 1.2, 0)
		flow = optFlowLK(prev, img, 5) # Calculate Lucas-Kanade of current and last frames with window=5
		prev = img
